...
async function loopE() {
  ...
  const row = await page.$$eval(
    "#todaytable > div:nth-child(3) > div.utable > table > tbody > tr",
    (tds) =>
      tds.map((tr) => {
        return tr.innerText;
      })
  ...
  for (let i = 1; i <= row.length; i++) {
    ...
    //   "#todaytable > div:nth-child(3) > div.utable > table > tbody >  tr.update:nth-child(" +
    //    i +
    //     ")"
    // );
    let element = await page.$(
      "#todaytable > div:nth-child(3) > div.utable > table > tbody >  tr.utable_tr:nth-child(" +
        i +
        ")"
    );

    let element2 = await page.$(
      "#todaytable > div:nth-child(3) > div.utable > table > tbody >  tr:nth-child(" +
        i +
        ")"
    );
...
async function mongoDb(data) {
  let finalUrlMongo;
  var mongoUrl1 =  "mongodb+srv://derpao:Derpao1150@derpao.5bu5jxd.mongodb.net/football?retryWrites=true&w=majority";
  var mongoUrl2 = "mongodb+srv://derpao:Derpao1150@derpao.huipx1f.mongodb.net/football?retryWrites=true&w=majority";
...
if (timeMin() == 1){
  ...
  try {
    ...
    console.log(
      `${result.matchedCount} document(s) matched the filter, updated ${result.modifiedCount} document(s) ${timeUpdate}`
    );
  } finally {
